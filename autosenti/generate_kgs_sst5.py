...
with open('../datasets/sst5/train.tsv', 'r', encoding='ISO-8859-1') as f:
    l_dict = f.readlines()
...
for i in range(1, len(l_dict)):
        ...
        if i == 'amod' or i == 'nsubj':
            if nlp.pos_tag(token[begin - 1])[0][1] in ['NN', 'NNS', 'NNP', 'NNPS'] and \
                ...
                if entity2 + '\t' + op not in op_dict.keys():
                    good_num, bad_num, won_num, ter_num = 0, 0, 0, 0
                    if judges == 'good':
                        good_num = 1
                    if judges == 'wonderful':
                        won_num = 1
                    ...
                    op_dict[entity2 + '\t' + op] = judges + '\t' + '1' + '\t' + '0' + '\t' + str(good_num) + '\t' + str(
                        bad_num) + '\t' + str(won_num) + '\t' + str(ter_num)

                else:
                    ac_judges, count_tr, conf, good_num, bad_num, won_num, ter_num = op_dict[entity2 + '\t' + op].split(
                        '\t')
    ...
    with open('./kgs/sst5.spo', 'w', encoding='utf-8') as f1:
