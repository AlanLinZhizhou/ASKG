...
# read sentences
with open('../datasets/mr/train.tsv', 'r', encoding='utf-8') as f:
    l_dict = f.readlines()
...
for i in range(1, len(l_dict)):
    ...
    if i % 500 == 0:
        print(i, ' examples,  ', len(l_dict) - i, 'to go')
            ...
            if nlp.pos_tag(token[begin - 1])[0][1] in ['NN', 'NNS', 'NNP', 'NNPS'] and \
                ...
                if entity2 + '\t' + op not in op_dict.keys():
                    ...
                    if judges == 'good':
                        ...
                        bad_num = 1
                    op_dict[entity2 + '\t' + op] = judges + '\t' + '1' + '\t' + '0' + '\t' + str(good_num) + '\t' + str(bad_num)
                    ...
                    # tell if there are conflicts
                    if conf == '1' or judges != ac_judges:
                        op_dict[entity2 + '\t' + op] = judges + '\t' + str(count_tr) + '\t' + '1' + '\t' + str(good_num) + '\t' + str(bad_num)
                    else:
                        op_dict[entity2 + '\t' + op] = judges + '\t' + str(count_tr) + '\t' + '0' + '\t' + str(good_num) + '\t' + str(bad_num)

    with open('./kgs/mr.spo', 'w', encoding='utf-8') as f1:
