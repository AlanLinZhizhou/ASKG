...
with open('../datasets/ALM/train.tsv', 'r', encoding='ISO-8859-1') as f:
    l_dict = f.readlines()
...
ent_skip_list = ['year', 'can', 'other', 'others', 'with', 'lots', 'ones', 'way', 'uses', 'use', 'time', 'kinds',
                 ...
                 'girl', 'something', 'hate', 'ages', 'words', 'me', 'old']
    ...
    for i, begin, end in dependencyParse:
        ...
        if i == 'amod' or i == 'nsubj':
            if nlp.pos_tag(token[begin - 1])[0][1] in ['NN', 'NNS', 'NNP', 'NNPS'] and \
                ...
                if len(entity2) < 10 or op.lower().strip() in op_skip_list or entity2.lower().strip() in ent_skip_list:
                    continue
                if op[-2:-1] in ['st', 'th']:
                    continue
                if len(op.split('-')) >= 3:
                ...
                if entity2 + '\t' + op not in op_dict.keys():
                    ang_num,  fear_num, joy_num, sad_num, surp_num = 0, 0, 0, 0, 0
                    if judges == 'angry':
                        ang_num = 1
                    if judges == 'fearful':
                        fear_num = 1
                    ...
                    op_dict[entity2 + '\t' + op] = judges + '\t' + '1' + '\t' + '0' + '\t' + str(ang_num) + '\t' + str(
                        fear_num) + '\t' + str(joy_num) + '\t' + str(sad_num) + '\t' + str(surp_num)

                else:
                    ac_judges, count_tr, conf, ang_num, fear_num, joy_num,  sad_num,surp_num = op_dict[entity2 + '\t' + op].split(
                        '\t')
